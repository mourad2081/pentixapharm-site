...
// ── 1. DEPENDENCIES REMOVAL ──────────────────────────────────────────────────
try {
  const pkgPath = path.join(B, 'package.json');
  let pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf8'));
  if (pkg.dependencies['@splinetool/react-spline']) delete pkg.dependencies['@splinetool/react-spline'];
  if (pkg.dependencies['@splinetool/runtime']) delete pkg.dependencies['@splinetool/runtime'];
  fs.writeFileSync(pkgPath, JSON.stringify(pkg, null, 2), 'utf8');
} catch (e) {
  console.log("Error removing spline from pkg:", e.message);
}

// ── 2. ADD IIS TO NAVBAR ─────────────────────────────────────────────────────
let nav = r('src/components/layout/Navbar.tsx');
if (!nav.includes('href: "/" + locale + "/iis"')) {
  nav = nav.replace(/\{ label: t\("stats"\), href: "\/" \+ locale \+ "\/stats" \},/g, 
...
if (!footer.includes('href: "/" + locale + "/iis"')) {
  footer = footer.replace(/\{ label: "Careers", href: "\/" \+ locale \+ "\/careers" \}/g, 
...
`);
console.log("IIS logic and pages written.");
