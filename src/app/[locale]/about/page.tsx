...
export default function AboutPage() {
  ...
  return (
    <div className="bg-[#F8FAFD] min-h-screen text-slate-800 pb-32">
      {/* ── HERO WITH IMAGE BG ─────────────────────────────────────────────── */}
      <section className="relative pt-44 pb-28 overflow-hidden bg-[#002A54] text-white shadow-2xl">
         ...
         <div className="container mx-auto px-6 max-w-6xl">
            <div className="grid md:grid-cols-3 gap-12">
               {[
                 ...
                 { icon: Shield, title: "Ethics", desc: "Absolute compliance with GCP/GLP and a commitment to transparent clinical reporting.", color: "text-[#002A54]" },
                 ...
                 <motion.div key={i} whileHover={{ y: -10 }} className="p-12 rounded-[4rem] bg-[#F8FAFD] border border-slate-100 group transition-all text-center">
                    ...
                    <h3 className="text-2xl font-heading font-extrabold text-[#002A54] mb-4 italic uppercase tracking-tighter">{v.title}</h3>
      ...
      <section className="py-32 bg-[#F8FAFD] border-y border-slate-100">
         <div className="container mx-auto px-6 max-w-6xl">
            <div className="text-center mb-24">
               ...
               <h2 className="text-5xl font-heading font-extrabold text-[#002A54] italic">A Legacy of <span className="text-teal">Precision</span></h2>
            </div>
            ...
            <div className="relative pl-12 border-l-2 border-slate-200 py-10 space-y-16 lg:space-y-24">
               {HISTORY.map((h, i) => (
                 <motion.div key={i} initial={{opacity:0, x:-20}} whileInView={{opacity:1, x:0}} viewport={{once:true}} transition={{delay: i*0.1}} className="relative">
                    <div className="absolute -left-[54px] top-6 w-7 h-7 rounded-full bg-white border-[6px] border-[#002A54] shadow-[0_0_20px_rgba(0,42,84,0.3)]" />
                    <div className="bg-white p-12 rounded-[3.5rem] shadow-xl border border-slate-50 flex flex-col md:flex-row gap-12 items-center hover:border-cyan/30 transition-all group">
                       ...
                       <div className="text-center md:text-left">
                          <h4 className="text-2xl font-heading font-extrabold text-[#002A54] mb-3 italic">{h.title}</h4>
        ...
        <div className="container mx-auto px-6 max-w-6xl">
          <motion.div initial="hidden" whileInView="show" viewport={{ once: true }} variants={stagger}>
            <div className="text-center mb-28">
              ...
              <h2 className="text-5xl md:text-7xl font-heading font-extrabold text-[#002A54] italic">The <span className="text-cyan">Executive</span> Board</h2>
            </div>
              ...
              {EXEC_BOARD.map((m, i) => (
                <motion.div key={i} variants={fadeUp} whileHover={{ y: -15 }} className="bg-[#F8FAFD] border border-slate-100 rounded-[4rem] p-14 hover:shadow-[0_40px_100px_-20px_rgba(0,42,84,0.15)] transition-all group relative overflow-hidden">
                  ...
                  <div className="flex flex-col items-center text-center mb-12 relative z-10">
                    <div className="w-32 h-32 rounded-[2.5rem] bg-[#002A54] flex items-center justify-center mb-10 shadow-2xl group-hover:rotate-6 transition-transform">
                      <span className="text-white font-heading font-extrabold text-5xl italic">{m.init}</span>
                    </div>
                    <h3 className="font-heading font-extrabold text-[#002A54] text-3xl italic tracking-tight">{m.name}</h3>
                  ...
                  <div className="flex justify-center items-center gap-6 border-t border-slate-200/50 pt-12 relative z-10">
                    <button className="w-14 h-14 rounded-full bg-white border border-slate-200 flex items-center justify-center text-slate-400 hover:text-[#002A54] hover:border-[#002A54] transition-all"><Linkedin className="w-6 h-6" /></button>
                    <button className="px-10 py-4 rounded-full bg-[#002A54] text-white text-[10px] font-extrabold uppercase tracking-widest hover:bg-cyan hover:text-navy transition-all shadow-xl">Full Profile <ExternalLink className="w-4 h-4" /></button>
                  </div>
                </motion.div>
              ))}
            </div>
        ...
        <div className="container mx-auto px-6 max-w-6xl">
          <motion.div initial="hidden" whileInView="show" viewport={{ once: true }} variants={stagger}>
            <div className="text-center mb-24">
              ...
              <h2 className="text-5xl font-heading font-extrabold text-[#002A54] italic">Supervisory <span className="text-teal text-transparent bg-clip-text bg-gradient-to-r from-teal to-cyan">Council</span></h2>
            </div>
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-10">
              {SUPERVISORY_BOARD.map((m, i) => (
                <motion.div key={i} variants={fadeUp} className="flex gap-10 items-start p-12 bg-white border border-slate-50 rounded-[3rem] hover:shadow-2xl transition-all group">
                  <div className="w-20 h-20 rounded-3xl bg-[#F8FAFD] flex-shrink-0 flex items-center justify-center font-heading font-extrabold text-[#002A54] group-hover:bg-[#002A54] group-hover:text-white transition-all shadow-inner text-2xl italic">
                    {m.init}
                  </div>
                  <div>
      ...
      <section id="sustainability" className="py-40 bg-white relative overflow-hidden group">
        ...
        <div className="container mx-auto px-6 relative z-10 max-w-6xl">
           <motion.div initial="hidden" whileInView="show" viewport={{ once: true }} variants={stagger} className="max-w-3xl">
              ...
              <h2 className="text-5xl md:text-7xl font-heading font-extrabold text-[#002A54] leading-tight mb-10 italic">Pioneering for <span className="text-teal">Sustainable</span> Healthcare</h2>
           ...
           <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-10">
              {[
                ...
                { title: "Governance", icon: Scale, desc: "Adhering to the Highest Prime Standard transparency as a Frankfurt-listed clinical-stage biotech.", color: "text-[#002A54]" }
              ].map((item, i) => (
                <motion.div key={i} whileHover={{y:-12}} className="bg-[#F8FAFD] p-16 rounded-[4rem] shadow-xl border border-slate-50 group transition-all">
                   <div className="w-20 h-20 rounded-3xl bg-white flex items-center justify-center mb-10 shadow-inner group-hover:bg-[#002A54] group-hover:text-white transition-all transform group-hover:rotate-12">
                      <item.icon className={"w-8 h-8 " + item.color} />
                   </div>
                   <h3 className="text-2xl font-heading font-extrabold text-[#002A54] mb-6 italic">{item.title}</h3>
